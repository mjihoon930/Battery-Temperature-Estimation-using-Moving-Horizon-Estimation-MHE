...
for i = 1:height(batteryData.Current)
    if batteryData.Current(i) < 0  % Negative current indicates charging
        ...
        if ((batteryData.Current(i)) < -4)
            ...
            if CC_startIndex < height(batteryData.Current) && Change_I_var > ChargeCurr * 0.95 && ~isCharging
                Change_I_var = abs(batteryData.Current(CC_startIndex) - batteryData.Current(restIndex));
...
legend('Observer Based Resistance', 'Voltage Drop Resistance', 'dataTable1.x60__30_second','dataTable1.x60__1_second')
%%
